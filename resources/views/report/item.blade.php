            ...
            <table class="w-full whitespace-no-wrap">
                <thead>
                    <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800" >
                        ...
                        <th class="px-4 py-3">Suplayer</th>
                        ...
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td class="px-4 py-3">
                                <div class="flex items-center text-sm">
                                    <div>
                                        <p class="font-semibold">{{ ucfirst($data->name) }}</p>
                                        @foreach ($merks as $merk)
                                            @if ($merk->id == $data->merk_id)
                                            <p class="text-xs text-gray-600 dark:text-gray-400">{{ $merk->name }}</p>
                                            @endif
                                        @endforeach
                                    </div>
                                </div>
                            ...
                            @foreach ($categories as $category)
                                ...
                                @endif
                            @endforeach
                            <td class="px-4 py-3 text-sm">{{ $data->qty }} pcs</td>
                            <td class="px-4 py-3 text-sm">{{ number_format($data->price,0,',','.') }} / pcs</td>
                            @foreach ($suppliers as $supplier)
                                @if ($supplier->id == $data->supplier_id)
                                    <td class="px-4 py-3 text-sm">{{ ucfirst($supplier->name) }}</td>
                                @endif
                            @endforeach
